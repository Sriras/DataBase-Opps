...
class Database_Operations():
    ...
    def __init__(self,localhost='localhost',password='Bullseye~12345',database_name=None,user='root',Table_name=None):
    ...
    def drop_databases(self,database_name):
        try:
            ...
            print('database {} droped sucessfully'.format(database_name))
    ...
    def create_tables(self):
        for elem in self.column_name.keys():
            ...
            self.dump_to_database(file_name=Stocks_scraper.StockInfo.stock_sting+'.csv')


    def dump_to_database(self,file_name):
        try:
            mydb=connections.Connect(host=self.localhost, user=self.user, passwd=self.password,database=self.database)
            print(mydb.is_connected(),'connection established')
            d=open(file_name, 'r')
            header=d.readline()
            data = csv.reader(d)
